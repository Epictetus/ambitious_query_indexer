...
class SQLParserTest < ActiveSupport::TestCase
  ...
  test "parser isn't confused by sql syntax as parts of other strings" do
    ...
    parse_data = SQLParser.parse(%q{SELECT * FROM airports WHERE airports.name = 'Manchester'})
  ...
  test "parser can understand joins in table references" do
    parse_data = SQLParser.parse(%q{SELECT `articles`.* FROM `articles` INNER JOIN `comments` ON `comments`.`article_id` = `articles`.`id`})
